...
import './Lodging.css'; // CSS 파일을 import합니다.
...
const Lodging = () => {
  ...
  const fetchLodgingData = async (retryCount = 3) => {
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        lodgingList.push({ title, addr1, firstimage });
      }

      console.log('Fetched data:', lodgingList);
      setLodgingData(lodgingList);
    } catch (error) {
  ...
  const totalPages = Math.ceil(lodgingData.length / itemsPerPage);
  const currentData = lodgingData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    ...
    <div className="container">
      ...
      <div className="lodging-list">
        {currentData.map((item, index) => (
          <div className="lodging-item" key={index}>
            <img className="lodging-image" src={item.firstimage} alt={item.title} />
            <div>
              <h2>{item.title}</h2>
              <p>{item.addr1}</p>
            </div>
